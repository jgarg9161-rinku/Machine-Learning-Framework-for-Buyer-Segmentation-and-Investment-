...
def plot_country_cluster_map(df: pd.DataFrame) -> go.Figure:
    # Map country codes to ISO-3 codes for Plotly compatibility
    country_mapping = {
        'USA': 'USA',
        'Canada': 'CAN',
        'UK': 'GBR',
    ...
    country_summary = (
        df_mapped.groupby('country_code')
          .agg(count=('country_code', 'size'))
          .reset_index()
    )
...
st.markdown(
    ...
    .hero {
        ...
        background: linear-gradient(135deg, rgba(15, 23, 42, 0.85) 0%, rgba(30, 41, 59, 0.85) 48%, rgba(51, 65, 85, 0.85) 100%), url('https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?auto=format&fit=crop&w=1400&q=80') center/cover no-repeat;
...
st.markdown('<div class="section-title scroll-fade"><h2>Market Snapshot</h2></div>', unsafe_allow_html=True)
row1, row2 = st.columns([2, 1])
with row1:
    st.markdown('<div class="section-card scroll-fade"><h3>Filtered Details</h3></div>', unsafe_allow_html=True)
    st.write(df_filtered[['unit_category', 'country', 'acquisition', 'floor_area_sqft', 'sale_price_numeric', 'satisfaction']].head(12))
with row2:
    st.markdown('<div class="section-card scroll-fade"><h3>Quick insight</h3></div>', unsafe_allow_html=True)
...
if show_raw:
    st.markdown('<div class="section-card scroll-fade"><h3>Raw Filtered Dataset</h3></div>', unsafe_allow_html=True)
    st.dataframe(df_filtered.reset_index(drop=True))

country_map_fig = plot_country_cluster_map(df_filtered)
if not df_filtered.empty:
    st.markdown('<div class=\"section-title scroll-fade\"><h2>Country Cluster Map</h2></div>', unsafe_allow_html=True)
...
if df_filtered.empty:
    ...
    if len(df_clean) < 10:
        ...
        if show_elbow:
            st.markdown('<div class="section-title scroll-fade"><h2>K-means Model Calibration</h2></div>', unsafe_allow_html=True)
            ...
            st.markdown('<div class="chart-card scroll-fade"></div>', unsafe_allow_html=True)
            st.pyplot(fig)

        pca_features, pca_model = compute_pca(scaled_features)

        if algorithm in ['K-means', 'Both']:
            st.markdown('<div class="section-title scroll-fade"><h2>K-means Clustering Results</h2></div>', unsafe_allow_html=True)
        ...
        if algorithm in ['Hierarchical', 'Both']:
            st.markdown('<div class="section-title scroll-fade"><h2>Hierarchical Clustering Results</h2></div>', unsafe_allow_html=True)
            ...
            if algorithm != 'K-means':
                st.markdown('<div class="section-title scroll-fade"><h2>Hierarchical Dendrogram</h2></div>', unsafe_allow_html=True)
    ...
    st.sidebar.write('Regions selected:')
    st.sidebar.write(', '.join(selected_regions[:10]) + ('...' if len(selected_regions) > 10 else ''))
